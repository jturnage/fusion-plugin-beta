    ...
    */
    ...
    module.exports = {
        ...
        videoTimestamp: data.videoTimestamp
            */
            ...
            //     takePictureFromFile(successCallback, errorCallback, args);
            // } else {
            exerciseSettings = JSON.parse(args[0]);
            systemSettings = JSON.parse(args[1]);
            takePictureFromCamera(successCallback, errorCallback, args);
            // }
        },
    };
    ...
    function takePictureFromCameraWindows (successCallback, errorCallback, args) {
        ...
        cameraCaptureUI.captureFileAsync(WMCapture.CameraCaptureUIMode.video).done(function (video) {
            if (!video) {
                successCallback('{"cancelled":true, "capturedVideo":false, "capturedImage":false, "videoUrl":null, "videoImage":null, "videoTimestamp":null}');
            }
            else {
    
                upload(video, successCallback, errorCallback);
                //video.copyAsync(Windows.Storage.ApplicationData.current.localFolder, video.name, Windows.Storage.NameCollisionOption.generateUniqueName).done(function (copiedFile) {
                //    var filename = copiedFile.name; //'ms-appdata:///local/' + copiedFile.name;
    
                //    Windows.Storage.ApplicationData.current.localFolder.getFileAsync(filename).then(file => {
                //        Windows.Storage.FileIO.readTextAsync(file).then(text => {
                //            upload(filename, text, successCallback, errorCallback);
                //        }, err => {
                //            errorCallback('{"cancelled":false, "capturedVideo":false, "capturedImage":false, "videoUrl":null, "videoImage":null, "videoTimestamp":null}');
                //        });
                //    }, err => {
                //        errorCallback('{"cancelled":false, "capturedVideo":false, "capturedImage":false, "videoUrl":null, "videoImage":null, "videoTimestamp":null}');
                //    });
        ...
        function upload(video, successCallback, errorCallback) {
            ...
            //headers.accept.append(new Windows.Web.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
            ...
            request += addFileInfo(video.name);
            ...
            Windows.Storage.FileIO.readBufferAsync(video).then(buffer => {
                ...
                client.postAsync(requestUri, content).then(response => {
                    ...
                    successCallback('{"cancelled":false, "capturedVideo":false, "capturedImage":false, "videoUrl":null, "videoImage":null, "videoTimestamp":null}');
                });
